...
Tests run automatically in CI at two stages:

| Stage | Trigger | Effect |
|-------|---------|--------|
| **GitHub Actions** | Every PR to `main` | "Run tests" is a required check — PR cannot be merged if tests fail |
| **Docker build (Render)** | Push to `main` (post-merge) | Tests run inside the Docker build — deploy is aborted if tests fail |
